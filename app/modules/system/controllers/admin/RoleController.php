...
class RoleController extends ZAdminController
{
    ...
    {
        //Get active module
        $core_module = CoreModules::find('published = 1');

        //Read resources
        foreach ($core_module as $cModule) {
            $filePath = ROOT_PATH . '/app/modules/' . $cModule->base_name . '/Resource.php';
            $module = str_replace(' ', '', $cModule->base_name);

            //Save rules from resource.php to database
            $resource = check_resource($filePath, $cModule->base_name);
        ...
        $allModuleBackEnd = get_child_folder(ROOT_PATH . '/app/modules/');
         ...
         */
        $unpublished_module = CoreModules::find('published = 0');
        $arrayModuleName = [];
    ...
    private function updateACLCache()
    {
        ...
        foreach ($roles as $role) {
            $query = 'SELECT module, controller, action, sub_action, mca FROM user_rules AS al INNER JOIN user_role_mapping AS alm ON alm.rule_id = al.rule_id WHERE alm.role_id = ' . $role->role_id;
